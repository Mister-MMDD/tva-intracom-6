...
sur plusieurs sessions successives :
- **Seuil OSS 10 000 €** : Appreciation sur l'année en cours et l'année précédente (N-1), avec propagation correcte du franchissement d'une année civile à l'autre dans un même traitement multi-fichiers. Les avoirs suivent désormais le régime (OSS ou DOMESTIC) de la vente d'origine même s'ils font repasser le cumul sous le seuil. Éligibilité OSS restreinte aux flux partant réellement du pays d'établissement du vendeur (Art. 59 ter 1.b).
- **IOSS_DIRECT vs DEEMED_SUPPLIER** : Le comportement par défaut devient `DEEMED_SUPPLIER` (Amazon redevable), `IOSS_DIRECT` est désormais un choix explicite via toggle. TVA nette IOSS (`net_ioss_vat`) tenant compte des remboursements, cohérente avec les totaux domestique/OSS.
- **Monaco** : Traitement des ventes et stocks à Monaco comme des ventes/mouvements domestiques français sur l'ensemble de la chaîne — seuil OSS, éligibilité OSS, base AIC (transferts de stock FBA, y compris les mouvements purement domestiques FR↔Monaco), ligne 18 CA3 (Convention fiscale franco-monégasque du 18/05/1963).
- **Taux de change OSS/IOSS** : Les deux régimes utilisent désormais systématiquement le taux BCE de **clôture de période** (Règl. UE 2020/194, art. 5 bis) — dashboard, récapitulatif Excel, export XML/CSV, et corrections a posteriori (avoirs rattachés à une période antérieure, valorisés au taux de clôture de la période d'origine, pas du jour de l'avoir). Correction du double arrondi lors des conversions de devises croisées (passage par un montant EUR intermédiaire en pleine précision).
- **Ligne 18 CA3 (Monaco)** : Remplissage de la ligne mémo dédiée (case 0038) sur le Cerfa 3310-CA3-SD.
- **VIES** : distinction entre un vrai downgrade suspect (réponse serveur vide, sans nom/adresse — dégradation VIES sous charge) et une vraie invalidation de numéro (réponse `valid=False` avec nom/adresse renseignés, à conserver comme information fiscale réelle). Purge de historique (`vies_check_history`, pas seulement les caches) des entrées corrompues par l'ancien bug de double préfixe pays. Restauration de l'affichage du dernier statut connu quand le cache est expiré et la revalidation échoue, sans jamais faire perdurer un statut VALIDE dans les calculs.
...
- **Concurrence & Verrous** : Utilisation de verrous avisés Postgres (`pg_advisory_xact_lock`) pour sécuriser les quotas SIREN et le verrouillage d'organisation, y compris en cas de réinitialisation de mot de passe concurrente.
- **Performance Postgres (BCE)** : `_db_get_rates_batch` matche désormais exactement les paires (devise, date) demandées via un `JOIN` sur `VALUES`, au lieu d'une plage de dates globale qui pouvait rapatrier des milliers de taux intermédiaires inutiles en RAM sur des fichiers aux ventes très espacées dans le temps.
...
- **Sécurité** : échappement HTML systématique (`html.escape()`) sur les champs libres injectés dans les rapports (nom d'entreprise, SIREN) ; suppression de la transmission du jeton de session Stripe dans les URLs de succès/annulation.
...
Détail complet de chaque correctif (fichier par fichier, avec justification
réglementaire et validation) : voir `README - evolution.md`.
...
- **2026-09-08 — Batch de 8 bugs confirmés (audit externe)** : anomalies de seuil/classification OSS, calculs AIC/Intrastat, cache VIES, format Amazon V5, crash de validation VIES, désynchronisation du toggle seuil N-1, gestion concurrente d'une réinitialisation de mot de passe. Un correctif distinct a également résolu un retrait de SIREN non immédiat pour une organisation hors-quota (blocage premium persistant). Détail dans `README - evolution.md`.
- **2026-09-06 — Paywall Stripe affiché à tort + crash import CSV Amazon (`ui/tabs/telechargements.py`, `ui/billing_gate.py`, `parsers/amazon/loader.py`, `parsers/amazon/classify.py`)** : deux bugs distincts remontés en usage réel.
  1. *Message paywall Stripe incohérent* — un compte déjà abonné mais bloqué pour une autre raison (conformité TVA/IOSS manquante, quota SIREN, rattachement compte Amazon, SIREN non reconnu) voyait quand même s'afficher le message de paiement Stripe, en plus du message d'erreur réel. Corrigé en n'affichant ce message que si `billing_ok=False` (compte réellement non payant). En creusant plus loin : un paiement par virement/prélèvement SEPA en cours de traitement (`sub_status == "incomplete"`) a aussi `billing_ok=False`, ce qui aurait affiché à tort le même message paywall à un utilisateur ayant déjà payé — corrigé par un message dédié (« paiement en cours de traitement »), à la fois sur ce banner et sur `preview_lock_message()` (tableaux/métriques masqués de tous les onglets). Nouvelle clé i18n `locked_payment_pending` (7 langues).
  2. *`AttributeError: 'NoneType' object has no attribute 'strip'` sur import CSV* — cause racine : `pl.read_csv()` représente par défaut une cellule CSV vide par `null` (pas une chaîne vide), pour n'importe quelle colonne, indépendamment de la casse de l'en-tête (déjà normalisée par ailleurs). Corrigé à la source avec `missing_utf8_is_empty_string=True`, plus une défense supplémentaire sur le repli `csv.DictReader` et les 2 points d'accès vulnérables de `classify.py`.
